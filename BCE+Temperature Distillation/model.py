...
def UpdateRepresentation(ResNet, task, train_Dataset, new_train_indexes, exemplar_indexes):
    ...
    for epoch in range(params.N_EPOCHS):
        ...
        for images, labels, indices  in train_loader:
            ...
            #  distillation loss
            if task>0:
                old_output = oldNet(images,features = False).to(params.DEVICE)
                soft_output = torch.log_softmax(output[:,:(task*params.TASK_CLASSES)]/params.T, dim=1)   # compute the log of softmax values
                soft_old_output = torch.softmax(old_output/params.T, dim=1)
                dist_loss = torch.sum(soft_output * soft_old_output, dim=1, keepdim=False)
                dist_loss = -torch.mean(dist_loss, dim=0, keepdim=False)
                dist_loss =  Variable(dist_loss.data, requires_grad=True).to(params.DEVICE)
                loss += dist_loss
            loss.backward()
            optimizer.step()
...
def ComputeMeansofExemplars(exemplar_indexes, ResNet,ds):           #you only need to do this once per task, that's why is out of classify method
    ...
    ResNet.train(False)
    exemplar_means = []
    for class_exemplars_indexes in exemplar_indexes:  #for each class
        mean=0
        ...
        nimages=0
        for image,_,_ in loader:      #features of all classes
            with torch.no_grad():
        ...
        mean=mean/nimages    #mean of the fatures of the current class
        mean = mean / mean.norm()
        mean=mean.cpu().numpy()
