...
def UpdateRepresentation(ResNet, task, train_Dataset, new_train_indexes, exemplar_indexes):
    ...
    classification=nn.CrossEntropyLoss() #BCE for classification loss and distillation loss
    distillation=torch.nn.KLDivLoss()
    ...
    for epoch in range(params.N_EPOCHS):
        ...
        for images, labels, indices  in train_loader:
            ...
            # classification loss
            if task==0:
                loss = classification(output,labels)
            # classification and distillation loss
            if task>0:
                m = nn.Softmax()
                l=  nn.LogSoftmax()
                class_loss= classification(output,labels)
                output_prob=l(output[:,:params.TASK_CLASSES*(task)])
                q = torch.zeros(50000, params.TASK_CLASSES*(task)).cuda()
                g = m(oldNet(images))
                q[indices] = g.data
                q_i = q[indices]
                dist_loss=distillation(output_prob,q_i)
