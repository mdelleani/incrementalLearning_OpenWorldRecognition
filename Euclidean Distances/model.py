...
def classify(features_batch, exemplar_means, nme = True):
    preds = []
    ...
    for features in features_batch:                                          #for each image
        ...
        top1 = np.min(distances)
        pred = np.argmin(distances)
        distances = np.delete(distances,np.argmin(distances))
        top2 = np.min(distances)
        margin = top2-top1
        batch_distances.append(top1)
        batch_margins.append(margin)
        preds.append(pred)                                         #take the nearest class
