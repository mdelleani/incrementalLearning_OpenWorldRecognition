...
class CosineLinear(Module):
    ...
    def forward(self, input):
        ...
        #print("X size: ", x.size())
        #print("X : ", x)
        y = F.normalize(self.weight, p=2, dim=1)
        #print("y size: ", y.size())
        #print("y : ", y)
        #y.to('cuda')
        #print(y)
...
class SplitCosineLinear(Module):
    ...
    def __init__(self, in_features, out_features1, out_features2, sigma=True):
        ...
        self.fc1 = CosineLinear(in_features, out_features1, False).to(params.DEVICE)
        self.fc2 = CosineLinear(in_features, out_features2, False).to(params.DEVICE)
        if sigma:
            self.sigma = Parameter(torch.Tensor(1))
    ...
    def forward(self, x):
        #print("X size:", x.size())
        #print("CUDA for X :", x.get_device())
        out1 = self.fc1(x)
        #print("Out1:", out1)
        #print("out1 size: ", out1.size())
        out2 = self.fc2(x)#.to("cpu")
        
        #print("Out2:", out2.get_device())
        #print("out2 size: ", out2.size())
        #print("params  ", params.DEVICE)
        #out2.to("cuda:0")
        #print("Out2.device", out2.get_device())
