...
print(params.LR)
ResNet=resnet.resnet32(num_classes=10)
...
for task in range(n_tasks):
  ...
  train_indexes =  train_Dataset.__getIndexesGroups__(task*10) # splits[task]

  test_indexes = test_indexes + test_Dataset.__getIndexesGroups__(task*10)
  ...
  for images, labels, _ in train_loader:
    ...
    # CNN
      ...
      preds_CNN = (torch.argmax(probabilities,1))

      n_images += len(images)
      all_preds = np.concatenate((all_preds,preds))
      all_labels = np.concatenate((all_labels,labels.cpu()))
      all_predsCNN = np.concatenate((all_predsCNN,preds_CNN.cpu()))


  correct_preds += (all_preds == all_labels).sum()
  accuracy = correct_preds/n_images
  print(f"accuracy on train set NME: {accuracy}")

  correct_predsCNN += (all_predsCNN == all_labels).sum()
  accuracyCNN = correct_predsCNN/n_images
  print(f"accuracy on train set CNN: {accuracyCNN}")
  ...
  correct_preds += (all_preds == all_labels).sum()
  accuracy = correct_preds/n_images
  print(f"accuracy on test set NME: {accuracy}")

  correct_predsCNN += (all_predsCNN == all_labels).sum()
  accuracyCNN = correct_predsCNN/n_images
  print(f"accuracy on test set CNN: {accuracyCNN}")
  #confusion matrix
